...
namespace MalbersAnimations.Controller.AI
{
    ...
    public class LookDecision : MAIDecision
    {
       ...
       // [Utilities.Flag]
        ...
        private void StoreColliders(MAnimalBrain brain, int index)
        {
            if (brain.DecisionsVars[index].gameobjects != null && brain.DecisionsVars[index].gameobjects.Length > 0)
            {
                ...
                for (int i = 0; i < brain.DecisionsVars[index].gameobjects.Length; i++)
                {
                    ...
                    // brain.DecisionsVars[index].Components[i] = brain.DecisionsVars[index].gameobjects[i].GetComponentInChildren<Collider>(); //Store the Renderer
                }
        ...
        public bool LookForTransformVar(MAnimalBrain brain, bool assign)
        {
            ...
            AssignMoveTarget(brain, transform.Value.transform, assign);
            return IsInFieldOfView(brain, transform.Value.position, out _);
        }
        ...
        public bool LookForGoVar(MAnimalBrain brain, bool assign)
        {
            ...
            return IsInFieldOfView(brain, gameObject.Value.transform.position, out _);
        }
        ...
        private bool IsInFieldOfView(MAnimalBrain brain, Vector3 Center, out float Distance)
        {
            ...
            Distance = Vector3.Distance(Center, brain.Eyes.position) * 0.95f * brain.Animal.ScaleFactor; 
            ...
            if (Distance < LookRange.Value) //Check if whe are inside the Look Radius
            {
                ...
                if (angle < LookAngle)
                //if (Vector3.Dot(Direction_to_Target.normalized, EyesForward) > Mathf.Cos(LookAngle * 0.5f * Mathf.Deg2Rad)) //Mean is in Look Range:
                {
              ...
              //  Debug.Log($"False  LOOK Angle {Vector3.Dot(Direction_to_Target.normalized, EyesForward)} < RANGE) {Mathf.Cos(LookAngle * 0.5f * Mathf.Deg2Rad)}");
                return false;
            }
        ...
        public bool LookForClosestWaypoint(MAnimalBrain brain, bool assign)
        {
            ...
            {
                var center = way.GetPosition() + new Vector3(0, way.Height, 0);
                if (IsInFieldOfView(brain, center, out float Distance))
                {
                    if (Distance < minDistance)
                    {
    ...
    public class LookDecisionEditor : Editor
    {
        ...
        SerializedProperty
            Description, UnityTag, debugColor, zoneType, ZoneID, tags, LookRange, LookAngle, lookFor, transform, gameobject, gameObjectSet, AllZones, WaitForTasks, WaitForTask,
